 ...
 */
PID Kernel_Create_Task_At(volatile PD *p, voidfuncptr f, int arg, PID pid, PRIORITIES py, TICK period, TICK wcet, TICK offset, TICK weight)
{
	unsigned char *sp;
...
/**
  *  Create a new task
  */
static PID Kernel_Create_Task(voidfuncptr f, int arg, PRIORITIES py, TICK period, TICK wcet, TICK offset, TICK weight)
{
	int x;
...
static void Next_Kernel_Request()
{
	...
	while (1)
	{
		...
		case CREATE:
			//  PORTA |= (1<<PA0);
			Kernel_Create_Task(Cp->code, Cp->arg, Cp->py_arg, Cp->period_arg, Cp->wcet_arg, Cp->offset_arg, Cp->weight_arg);
...
/**
  ...
  * Task_Next().
  */
PID Task_Create_WRR(voidfuncptr f, int arg, TICK weight)
{
	if (KernelActive)
	{
		Disable_Interrupt();
...
*/
	...
	{
		Disable_Interrupt();
		Cp->duration = Cp->duration + 1;
		if (Cp->duration >= Cp->weight)
		{
			Cp->duration = 0;
			Cp->request = NEXT;
		}
